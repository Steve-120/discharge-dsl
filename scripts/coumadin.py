...
class NoCoumadin(BaseModel):
    needed: Literal[False]

class CoumadinNeeded(BaseModel):
    needed: Literal[True]
    indication: str
    inr_goal: Tuple[float, float]

CoumadinOutput = Union[NoCoumadin, CoumadinNeeded]

class CoumadinSignature(dspy.Signature):
    """
    Read the given patient data who has undergone CABG surgery, then determine if the patient needs to be discharged with instructions regarding Coumadin/Warfarin use. Include your confidence level, and evidence to support your answer.
    """
    ...
    coumadin_output: CoumadinOutput = dspy.OutputField(desc=\
"""Whether or not Coumadin/Warfarin instructions are needed. If needed, provide the indication in a few words,
and the desired range of INR.""")
...
class CoumadinMM(dspy.Module):
    ...
    def get_prompt_inputs(self, subject_id: int, hadm_id: int):
        ...
        prompts["medications"] = mh.get_medications_text(selected_meds=coumadin_meds)

        return prompts

    def forward(self, subject_id: int, hadm_id: int):
        prompts = self.get_prompt_inputs(subject_id, hadm_id)
        response = self.refined_coumadin_module(
...
def get_coumadin_answer_at_index(program, i):
